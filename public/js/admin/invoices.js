...
function renderInvoicesFromMemory() {
    ...
    if (currentFilter === 'PENDING') {
        filtered = filtered.filter(inv => 
            inv.billingStatus !== 'COMPLETED' && 
            inv.billingStatus !== 'CANCELLED' && 
            inv.billingStatus !== 'CANCELADO' && 
            !['CANCELADO', 'RECHAZADO'].includes(inv.status)
        );
...
window.filterTab = (status) => {
    ...
    document.querySelectorAll('.filter-tab').forEach(btn => {
        btn.classList.remove('border-b-2', 'border-brand-orange', 'text-brand-black', 'bg-white');
    ...
    if(activeBtn) {
        ...
        activeBtn.classList.add('border-b-2', 'border-brand-orange', 'text-brand-black', 'bg-white');
    }
...
function renderInvoiceCard(order) {
    ...
    div.innerHTML = `
        ...
        <div class="w-full md:col-span-2 md:text-right flex justify-between md:block">
            ...
            <span class="text-sm font-black text-brand-orange">$${(order.total || 0).toLocaleString('es-CO')}</span>
        </div>
        ...
        <div class="w-full md:col-span-2 flex justify-end gap-2">
            ...
            ${isCompleted && order.invoiceUrl ? `
                <a href="${order.invoiceUrl}" target="_blank" class="w-8 h-8 rounded-lg bg-gray-50 border border-gray-200 text-gray-500 hover:text-brand-orange hover:border-brand-orange flex items-center justify-center transition" title="Ver PDF">
            ...
            ` : `
                <button onclick="window.openUploadModal('${order.id}')" class="w-8 h-8 rounded-lg bg-brand-black text-white hover:bg-brand-orange hover:text-brand-black transition flex items-center justify-center" title="Subir Factura">
