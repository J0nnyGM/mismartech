...
function lockGlobalInputs() { 
    ...
    stockLabel.innerHTML = "Stock Total <span class='text-xs text-brand-orange'>(Auto)</span>";
        ...
        div.innerHTML = `<img src="${src}" class="w-full h-full object-cover">
            <div class="absolute inset-0 bg-brand-black/60 opacity-0 group-hover:opacity-100 transition flex flex-col items-center justify-center gap-2">
                <div class="flex gap-2">
                    <button type="button" onclick="moveGlobalImage(${index}, -1)" class="w-8 h-8 rounded-lg bg-white text-brand-black hover:bg-brand-orange transition"><i class="fa-solid fa-arrow-left text-xs"></i></button>
                    <button type="button" onclick="moveGlobalImage(${index}, 1)" class="w-8 h-8 rounded-lg bg-white text-brand-black hover:bg-brand-orange transition"><i class="fa-solid fa-arrow-right text-xs"></i></button>
                </div>
...
form.onsubmit = async (e) => {
    ...
    try {
        ...
        const updateData = {
            ...
            mainImage: finalGlobalUrls[0] || Object.values(colorUrlsMap).find(arr => arr && arr.length > 0)?.[0] || ""
        };
