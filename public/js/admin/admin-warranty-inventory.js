...
window.setView = (mode) => {
    ...
    if(mode === 'active') {
        tabActive.classList.add('active', 'text-brand-orange', 'border-brand-orange');
        tabActive.classList.remove('text-gray-400', 'border-transparent');
        tabHistory.classList.add('text-gray-400', 'border-transparent');
        tabHistory.classList.remove('active', 'text-brand-orange', 'border-brand-orange');
    } else {
        tabHistory.classList.add('active', 'text-brand-orange', 'border-brand-orange');
        tabHistory.classList.remove('text-gray-400', 'border-transparent');
        tabActive.classList.add('text-gray-400', 'border-transparent');
        tabActive.classList.remove('active', 'text-brand-orange', 'border-brand-orange');
    }
...
function renderGroupedInventory(items) {
    ...
    const htmlBuffer = Object.values(groups).map(group => {
        const unitsHTML = group.units.map(unit => {
            ...
            if(unit.status === 'ENTREGADO' || unit.status === 'FINALIZADO') badgeColor = "bg-brand-orange/20 text-brand-orange";

            const actionBtn = currentView === 'active' 
                ? `<button onclick="window.openStatusModal('${unit.id}')" class="bg-brand-black text-white px-4 py-2 rounded-xl text-[9px] font-black uppercase hover:bg-brand-orange hover:text-brand-black transition shadow-sm">Gestionar</button>`
            ...
            <tr class="border-b border-gray-50 last:border-0 hover:bg-slate-50 transition item-row-searchable">
                <td class="p-4 w-48">
                    <p class="font-mono text-[10px] font-bold text-brand-orange bg-brand-orange/5 px-2 py-1 rounded w-fit select-all searchable-sn">${unit.sn}</p>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                                ${currentView === 'active' ? 'Stock Activo' : 'Historial'}: <span class="text-brand-orange text-sm">${group.count}</span>
                            </p>
                        </div>
                    </div>
                </div>
