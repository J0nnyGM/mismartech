            ...
            product.combinations.forEach((c, index) => {
                ...
                html += `
                    <div class="flex justify-between items-center gap-3 p-2 bg-white rounded-lg border border-gray-100 shadow-sm">
                        ...
                        <div class="w-1/2"><input type="text" class="var-discount-input w-full bg-slate-50 border border-gray-200 rounded-md p-2 text-xs font-bold text-brand-orange outline-none focus:border-brand-orange focus:bg-white transition" data-index="${index}" value="${formatCurrency(cCurrent)}" placeholder="$ 0"></div>
                    </div>`;
            });
