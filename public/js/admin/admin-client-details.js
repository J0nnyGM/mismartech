...
async function loadClientOrders(isNextPage = false) {
    ...
    if (!isNextPage) {
        els.ordersList.innerHTML = `<tr><td colspan="6" class="p-8 text-center"><i class="fa-solid fa-circle-notch fa-spin text-brand-orange"></i> Cargando...</td></tr>`;
        ...
        snapHist.forEach(d => {
            ...
            const row = `
                <tr class="hover:bg-slate-50 transition border-b border-gray-50 group animate-in fade-in">
                    <td class="px-8 py-6 font-mono text-xs font-bold text-brand-orange">#${(o.internalOrderNumber || d.id.slice(0,6)).toString().toUpperCase()}</td>
                    ...
                    <td class="px-8 py-6 text-center">
                        <button onclick="window.openOrderModal('${d.id}')" title="Ver Detalle" class="w-8 h-8 flex items-center justify-center mx-auto rounded-lg bg-white border border-gray-200 text-gray-400 hover:text-brand-orange hover:border-brand-orange transition shadow-sm"><i class="fa-solid fa-eye text-xs"></i></button>
                    </td>
                </tr>
            `;
...
async function loadClientSerials(isNextPage = false) {
    ...
    if (!isNextPage) {
        container.innerHTML = '<div class="col-span-full text-center text-gray-400 py-10"><i class="fa-solid fa-circle-notch fa-spin text-brand-orange text-2xl mb-2"></i><p class="text-xs font-bold uppercase tracking-widest">Descargando seriales...</p></div>';
        clientLoadedSerials = [];
    ...
    container.innerHTML = serialsArray.map(s => {
        ...
        return `
            <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg transition-all relative group overflow-hidden fade-in">
                <div class="absolute top-0 left-0 w-1 h-full bg-brand-orange group-hover:w-2 transition-all"></div>
                <div class="flex justify-between items-start mb-3 pl-2">
                    <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest"><i class="fa-solid fa-tag text-brand-orange mr-1"></i> Serial Registrado</p>
