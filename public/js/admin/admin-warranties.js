...
function renderWarrantyRow(w) {
    ...
    let actionBtn = `<button onclick="window.openManageModal('${w.id}')" class="bg-brand-black text-white px-4 py-2 rounded-lg text-[9px] font-black uppercase hover:bg-brand-orange hover:text-brand-black transition shadow-md">Gestionar</button>`;
            ...
            <td class="px-8 py-6">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p class="text-[9px] font-mono text-brand-orange font-bold">${w.snProvided}</p>
                    </div>
                </div>
            </td>
    ...
    if (w.status !== 'PENDIENTE_REVISION' && w.status !== 'PENDIENTE') {
        ...
        if(w.technicalReportUrl) {
            if(!linkLabel) {
                ...
                linkLabel.className = "text-[10px] font-bold text-brand-orange hover:underline mt-1 block";
