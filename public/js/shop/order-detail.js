...
function renderTimeline(currentStatus) {
    ...
    steps.forEach((step, index) => {
        ...
        if (isCurrent) {
            circleClass = "bg-brand-black text-brand-orange border-4 border-brand-orange shadow-xl shadow-cyan-500/30 scale-110 z-20";
            iconClass = "fa-beat-fade"; 
            textClass = "text-brand-orange font-black transform scale-105";
        } else if (isCompleted) {
...
function renderPaymentInfo(order) {
    ...
    let icon = "fa-credit-card"; let label = "Tarjeta / Electrónico"; let iconColor = "text-brand-orange"; let bgIcon = "bg-brand-orange/10";
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start mb-2">
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2 mb-2">
                            ...
                            ${item.capacity ? `<span class="text-[8px] font-bold uppercase bg-cyan-50 border border-cyan-100 px-2 py-0.5 rounded text-brand-orange">${item.capacity}</span>` : ''}
