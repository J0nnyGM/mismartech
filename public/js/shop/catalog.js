...
// --- OVERLAY LÓGICA (Idéntico a shop/search.js) ---
                    ...
                    ${sec.items.map(item => {
                        ...
                        <div class="flex items-center gap-3 group cursor-pointer hover:bg-slate-50 p-1.5 rounded-xl transition">
                            <input type="checkbox" id="${sec.id}-${item.label}" value="${item.label}" 
                                class="filter-checkbox appearance-none w-4 h-4 border-2 border-gray-200 rounded-md checked:bg-brand-orange checked:border-brand-orange transition cursor-pointer shrink-0"
                            ...
                            <label for="${sec.id}-${item.label}" class="flex-grow flex justify-between items-center cursor-pointer select-none">
                                <span class="text-[11px] font-bold text-gray-600 uppercase tracking-wide group-hover:text-brand-orange transition truncate mr-2">${item.label}</span>
