            ...
            <div class="bg-white p-6 rounded-[2rem] border ${isDef ? 'border-brand-orange shadow-md shadow-brand-orange/10' : 'border-gray-100 hover:border-gray-200'} shadow-sm relative group transition-all">
                <div class="flex justify-between items-start mb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div><h4 class="font-black text-xs uppercase tracking-tight text-brand-black">${addr.alias}</h4>${isDef ? '<span class="text-[8px] font-black uppercase text-brand-orange">Principal</span>' : ''}</div>
                    </div>
...
function renderOrdersList(orders) {
    ...
    if (pageOrders.length === 0) {
        container.innerHTML = `
            <div class="bg-white p-10 rounded-[2rem] text-center border-2 border-dashed border-gray-200">
                ...
                <a href="/shop/catalog.html" class="inline-block mt-4 text-brand-orange font-bold text-xs hover:underline">Ir a la tienda</a>
            </div>`;
        return;
    }
    ...
    pageOrders.forEach(order => {
        ...
        card.className = "bg-white rounded-[1.5rem] p-5 border border-gray-100 shadow-sm hover:border-brand-orange/30 hover:shadow-md transition-all cursor-pointer group relative overflow-hidden mb-4";
        ...
        card.innerHTML = `
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pl-4">
                <div class="flex items-center gap-4">
                    <div class="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center text-brand-black font-black text-xs border border-gray-100 group-hover:bg-brand-black group-hover:text-brand-orange transition-colors shadow-sm">
                    ...
                    <div>
                        ...
                        <p class="text-xs font-bold text-gray-600 group-hover:text-brand-orange transition-colors">${totalItems} producto(s)</p>
                    </div>
                </div>
                <div class="flex items-center justify-between w-full md:w-auto gap-8 mt-2 md:mt-0 pl-16 md:pl-0 border-t md:border-0 border-gray-50 pt-3 md:pt-0">
                    ...
                    <div class="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-gray-300 group-hover:bg-brand-orange group-hover:text-white transition shadow-sm"><i class="fa-solid fa-chevron-right text-xs"></i></div>
                </div>
            </div>`;
    ...
    try {
        ...
        deptInput.oninput = (e) => {
            ...
            allDepartments.filter(d => d.name.toLowerCase().includes(term)).forEach(dept => {
                ...
                div.className = "p-2 hover:bg-slate-50 cursor-pointer text-xs font-bold rounded-lg transition uppercase text-gray-600 hover:text-brand-orange";
